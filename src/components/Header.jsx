...
import { getWixTokens } from '../lib/wixClient';
  ...
  useEffect(() => {
    const sync = async () => {
      try {
        ...
        if (!wixTokens?.accessToken?.value) { setCount(0); return; }
        const res = await base44.functions.invoke('wixCart', { action: 'get', wixTokens });
