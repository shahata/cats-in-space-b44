    ...
    <div className="min-h-screen bg-background font-body">
      ...
      <div className="pt-28 px-[6vw] md:px-[8vw] pb-20">
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 + i * 0.1 }}>
                <div className="bg-card border border-border p-6 h-full flex flex-col">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-xs font-mono text-primary/60 tracking-widest">{f.deck}</span>
                  </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            { emoji: '🌍', title: 'Planet Database', desc: 'All surveyed worlds ranked by habitability for feline life.', link: '/planets', linkText: 'Explore Planets →' },
            { emoji: '🧑‍🚀', title: 'Crew Roster', desc: 'The bravest cats to ever leave a perfectly good cardboard box behind.', link: '/crew', linkText: 'Meet the Crew →' },
            { emoji: '🚀', title: 'Mission Control', desc: 'Tracking every whisker-raising expedition into the unknown.', link: '/missions', linkText: 'All Missions →' },
          ].map((item, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + i * 0.1 }}>
              <Link to={item.link} className="block bg-card border border-border hover:border-primary/40 p-6 text-center group transition-colors h-full">
                ...
                <span className="text-primary text-sm font-mono">{item.linkText}</span>
              </Link>
            </motion.div>
          ))}
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }}
          className="mt-16 text-center border border-border bg-card p-12">
